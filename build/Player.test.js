...
test("checking recieving hits", () => {
  ...
  human.recieveAttack(1);
  expect(human.gameboard.isPositionHit(1)).toBe(true);
});
test("checking attacking", ()=>{
    computer.setShip(1,5);
    expect(computer.gameboard.getPoint(1).isHit).toBe(false)
    human.beginAttack(1);
    expect(computer.gameboard.getPoint(1).isHit).toBe(true)
})
test("checking loss detection", () => {
  expect(human.hasLost()).toBe(false);
  human.recieveAttack(11);
  human.recieveAttack(21);
